...
class AjaxHandler implements \VendorFPF\WPDesk\PluginBuilder\Plugin\HookablePluginDependant
{
    ...
    public function processAjaxNoticeDismiss()
    {
        if (isset($_POST[self::POST_FIELD_NOTICE_NAME])) {
            ...
            $option_name = \VendorFPF\WPDesk\Notice\PermanentDismissibleNotice::OPTION_NAME_PREFIX . $noticeName;
            if (\wp_verify_nonce($security, $option_name)) {
                \update_option($option_name, \VendorFPF\WPDesk\Notice\PermanentDismissibleNotice::OPTION_VALUE_DISMISSED);
