...
if( isset( $_POST[ 'action' ] ) && $_POST[ 'action' ] != '' ) {
    ...
    elseif( $action == 'oci_install_dependancy' ) : // action to install plugins [Not in use]
        ...
        //check is onecom theme is activated or not
        $theme = wp_get_theme();
        $theme_status = 'onecom_theme';
        if ( 'one.com' !== strtolower( $theme->display( 'Author', false ) ) ) {
            $theme_status = 'not_onecom_theme';
        }

        $activate_plugins = oci_activate_plugins();
        $response[ 'activate' ] = $activate_plugins;
        $response['loginUrl'] = wp_login_url();
        $response['siteUrl'] = site_url();
        $response['themeStatus'] = $theme_status;
        $response['message'] = __('Finalising...','oci');
    ...
    elseif( $action == 'oci_check_if_busy' ):
        ...
        echo json_encode( $response );
        wp_die();
    elseif($action == 'ocigeneratePassword'):
        $initial_password = wp_generate_password( 12 );
        if(!empty($initial_password) || $initial_password != null){
            $response[ 'type' ] = 'success';
            $response[ 'message' ] = 'password generated';
            $response[ 'passwd' ] =  $initial_password;
        }else{
            $response[ 'type' ] = 'error';
            $response[ 'message' ] = __( 'password generation failed', 'onecom-wp' );
            $response[ 'passwd' ] = '';
        }
        echo json_encode( $response );
        wp_die();
